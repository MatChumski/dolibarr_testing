...
/**
 * \file    test/phpunit/SociNexusFunctionalTest.php
 * \ingroup socinexus
 ...
 *
 * @package Testsocinexus
 */
class SociNexusFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modSociNexus')->click();
		} else {
			// Disable the module
			$this->byHref('modSociNexus')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/socinexus/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('socinexus/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/socinexus/admin/about.php');
		$this->authenticate();
		return $this->assertContains('socinexus/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('socinexuswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modSociNexus_SociNexusTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modSociNexus_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}

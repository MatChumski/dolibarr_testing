...
/**
 * \file    garantias/admin/setup.php
 * \ingroup garantias
 * \brief   Garantias setup page.
 */
...
require_once '../lib/garantias.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "garantias@garantias"));
...
$arrayofparameters = array(
	'GARANTIAS_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'GARANTIAS_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/garantias/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'GARANTIAS_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'GARANTIAS_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$page_name = "GarantiasSetup";
llxHeader('', $langs->trans($page_name));

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($page_name), $linkback, 'object_garantias@garantias');

// Configuration header
$head = garantiasAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "garantias@garantias");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("GarantiasSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'GARANTIAS_'.strtoupper($myTmpObjectKey).'_ADDON';

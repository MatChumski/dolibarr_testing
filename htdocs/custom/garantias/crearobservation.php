...
// Se obtiene el ID de la intervencion en la que vamos a agregar una observacion
if  (isset($_GET['id'])) {
  $id = $_GET['id'];
}
    ...
    if (!empty($_FILES["archivo"])){
        ...
        for($x=0; $x<count($_FILES["archivo"]["name"]); $x++){
            ...
            if ($tipo != 'image/jpeg' && $tipo != 'image/jpg' && $tipo != 'image/png' && $tipo != 'image/gif' && $tipo != 'image/bmp' && $tipo != 'image/svg' && $tipo != 'image/tiff'){
                ...
                $query = "INSERT INTO llx_observation (date, observation, responsable, imagen, fk_revision, status) 
                VALUES ('$fecha_actual', '$observation', '$responsable', '$destino', '$id', 1)";
        ...
        //Sentencia SQL
        //Se cargan los datos en la variable $query para ser enviada a la base de datos
        $query = "INSERT INTO llx_observation (date, observation, responsable, fk_revision, status) 
        VALUES ('$fecha_actual', '$observation', '$responsable', '$id', 1)";
        mysqli_query($conn, $query);//se actualizan en la base de datos
        //Redireccionamos a la hoja de vida que estamos trabajando
        header("Location: intervenciones.php?id=$idh");
...
<html>
    ...
    <body>
        ...
        <form action="crearobservation.php?id=<?php echo $id?>&idh=<?php echo $idh?>" method="POST" enctype="multipart/form-data">
            <h4>PROCEDIMIENTO INTERNO</h4></br></br>
            <!-- Se imprime un menu desplegable para seleccionar el tecnico responsable de realizar la observacion de la intervencion-->
            <label>Responsable </label>
            <select name="responsable"><?php
            //Se consulta en la base de datos el nombre y apellido de los que se encuentren en el grupo de tecnicos
            ...
            while($row = mysqli_fetch_assoc($group)) {?>
                <option value="<?php echo $row['nombre']; echo" "; echo $row['apellido'];?>"><?php echo $row['nombre']; echo" "; echo $row['apellido'];?></option>
            <?php } ?>
            </select></br></br>
            <!-- Se imprime un textarea para escribir el procedimiento o nota de lo realizado -->
            ...
            <!-- Boton para cancelar y regresar a la visualizacion de la hoja de vida-->

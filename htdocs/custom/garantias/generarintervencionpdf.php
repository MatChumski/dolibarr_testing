...
<html>
    ...
    <body>
        <?php
        //Se consulta en la base de datos todos las intervenciones pertenecientes a la hoja de vida
        $query = "SELECT * FROM llx_revision WHERE fk_equipment_history = $id AND status=1";
        $result = mysqli_query($conn, $query);
        //while para imprimir todas las intervenciones
        while($row = mysqli_fetch_assoc($result)) { ?>
            <br></br>
            <!-- Se imprime la fecha de intervencion-->
            ...
            <table class="table" VALIGN="TOP" style="text-align: center" width="100%" border-collapse="collapse"><!-- inicio de la subtabla de observaciones-->
                ...
                <tbody>
                    <?php
                    //impresion de todas las observaciones de cada intervenecion
                    ...
                    while($row2 = mysqli_fetch_assoc($result2)) {
                        ...
                        $query="SELECT imagen FROM llx_observation WHERE date='$date' AND rowid!='$rowid' AND fk_revision=$idrevision";
                        ...
                        if (mysqli_num_rows($result3) >= 1){ ?>
                            <tr VALIGN="TOP">
                                ...
                                //Se imprime imagen
                                echo '<img src="'.$row2['imagen'].'" width="160" height="160">';
                                while($row3 = mysqli_fetch_assoc($result3)) {
                                    echo "<br></br>";
                                    //se imprimen las demas imagenes que se encontraron con la misma fecha de subida
                                    echo '<img src="'.$row3['imagen'].'" width="160" height="160">';
                                    $cont=$cont+1;
                                }
                            ...
                            <tr VALIGN="TOP">
                                ...
                                <td><?php echo $responsable=$row2['responsable']; ?></td>
                                <td style="text-align: justify"><?php echo $row2['observation']; ?></td>
                                <td><?php echo '<img src="'.$row2['imagen'].'" width="160" height="160">';?></td>
                            </tr><!-- se cierra la fila -->
                        <?php } ?>
                    <?php } ?>
                </tbody>
...
//$mipdf->load_html(utf8_encode(ob_get_clean())); 
...
$mipdf->stream("IntervencionesCompleta-$ref.pdf", array("Attachment" => false));

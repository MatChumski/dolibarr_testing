...
/**
 * \file    test/phpunit/GarantiasFunctionalTest.php
 * \ingroup garantias
 ...
 *
 * @package Testgarantias
 */
class GarantiasFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modGarantias')->click();
		} else {
			// Disable the module
			$this->byHref('modGarantias')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/garantias/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('garantias/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/garantias/admin/about.php');
		$this->authenticate();
		return $this->assertContains('garantias/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('garantiaswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modGarantias_GarantiasTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modGarantias_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}

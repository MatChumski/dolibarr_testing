...
CASE
	WHEN fac.type = '2' AND fac.total_ht < 0 THEN TRUNCATE((fac.total_ht * -1), 2)
	ELSE TRUNCATE(fac.total_ht, 2)
END AS LineExtensionAmount,
CASE
	WHEN fac.type = '2' AND fac.total_tva < 0 THEN TRUNCATE((fac.total_tva * -1), 2)
	ELSE TRUNCATE(fac.total_tva, 2)
END AS tva,
CASE
...
while($row = $result->fetch_array(MYSQLI_ASSOC)) {
	...
	if($validaciones) {
		...
		CASE
			WHEN fac.type = '0' AND det.total_tva > 0   THEN TRUNCATE(det.total_ht, 2)
			WHEN fac.type = '0' AND det.total_tva <= 0  THEN TRUNCATE(det.total_tva, 2)
			WHEN fac.type = '2' AND det.total_tva < 0   THEN TRUNCATE((det.total_ht * -1), 2)
			WHEN fac.type = '2' AND det.total_tva >= 0  THEN TRUNCATE(det.total_tva, 2)
			WHEN fac.type = '3' AND det.total_tva > 0   THEN TRUNCATE(det.total_ht, 2)
			WHEN fac.type = '3' AND det.total_tva <= 0  THEN TRUNCATE(det.total_tva, 2)

...
CASE
	WHEN fac.type = '2' AND fac.total < 0 THEN TRUNCATE((fac.total * -1), 2)
	ELSE TRUNCATE(fac.total, 2)
END AS LineExtensionAmount,
CASE
	WHEN fac.type = '2' AND fac.tva < 0 THEN TRUNCATE((fac.tva * -1), 2)
	ELSE TRUNCATE(fac.tva, 2)
END AS tva,
CASE
	WHEN fac.type = '2' AND fac.total_ttc < 0 THEN TRUNCATE((fac.total_ttc * -1), 2)
...
CASE 
	WHEN fac.type = 0 THEN 'FA'
	WHEN fac.type = 2 THEN 'NC'
	WHEN fac.type = 3 THEN 'ND'
...
WHERE (NOT extra.envio OR extra.envio is NULL) AND LEFT(fac.ref,1) <> '(' AND (extra.isvalid <> 'true' OR extra.isvalid is NULL) AND fac.rowid IN (".ROWIDS.") ORDER BY fac.ref ";
...
while($row = $result->fetch_array(MYSQLI_ASSOC)) {
	...
	if($row['DocumentTypeCode'] == 'NC' OR $row['DocumentTypeCode'] == 'ND') {
	...
	SELECT 
	CASE
		WHEN fac.type = '0' THEN TRUNCATE(SUM(det.total_ht), 2)
		WHEN fac.type = '2' THEN TRUNCATE(SUM((det.total_ht * -1)), 2)
		WHEN fac.type = '3' THEN TRUNCATE(SUM(det.total_ht), 2)
	END AS TaxExclusiveAmount
		FROM ".PREFIJO."facturedet AS det
		INNER JOIN ".PREFIJO."facture AS fac
		ON det.fk_facture  = fac.rowid
		WHERE det.tva_tx > 0 AND det.fk_facture = ".$row['rowid'];
	...
	if($validaciones) {
		...
		$curl.= '","DocumentTypeCode":"'.$row['DocumentTypeCode'];                             // FA = Factura, NC = NotaCredito, ND=NotaDebito 
		...
		$curl.= '","ReceptorName":"'.str_replace(array('\'', '"'), '', $row['ReceptorName']);
		...
		$curl.= '","ContactName":"'.str_replace(array('\'', '"'), '', $row['ContactName']);
		...
		CASE
			WHEN fac.type = '0' THEN TRUNCATE(det.total_ht, 2)
			WHEN fac.type = '2' AND det.total_ht < 0  THEN TRUNCATE((det.total_ht * -1), 2)
			WHEN fac.type = '2' AND det.total_ht >= 0 THEN TRUNCATE(det.total_ht, 2)
			WHEN fac.type = '3' THEN TRUNCATE(det.total_ht, 2)
		END AS LineExtensionAmount,	
		CASE
			WHEN fac.type = '0' THEN TRUNCATE(det.total_tva, 2)
			WHEN fac.type = '2' AND det.total_tva < 0  THEN TRUNCATE((det.total_tva * -1), 2)
			WHEN fac.type = '2' AND det.total_tva >= 0 THEN TRUNCATE(det.total_tva, 2)
			WHEN fac.type = '3' THEN TRUNCATE(det.total_tva, 2)
		END AS TaxAmount,
		'false' AS TaxEvidenceIndicator,
		CASE
			WHEN fac.type = '0' AND det.total_tva > 0   THEN TRUNCATE(total_ht, 2)
			WHEN fac.type = '0' AND det.total_tva <= 0  THEN TRUNCATE(total_tva, 2)
			WHEN fac.type = '2' AND det.total_tva < 0   THEN TRUNCATE((det.total_ht * -1), 2)
			WHEN fac.type = '2' AND det.total_tva >= 0  THEN TRUNCATE(det.total_tva, 2)
			WHEN fac.type = '3' AND det.total_tva > 0   THEN TRUNCATE(total_ht, 2)
			WHEN fac.type = '3' AND det.total_tva <= 0  THEN TRUNCATE(total_tva, 2)
		END AS TaxableAmount,
		TRUNCATE(det.tva_tx, 2) AS Percent,
		'01' AS TaxSchemeID,
		CASE
			WHEN fac.type = '0' THEN TRUNCATE(det.subprice, 2)
			WHEN fac.type = '2' AND det.subprice <  0  THEN TRUNCATE((det.subprice * -1), 2)
			WHEN fac.type = '2' AND det.subprice >= 0  THEN TRUNCATE(det.subprice, 2)
			WHEN fac.type = '3' THEN TRUNCATE(det.subprice, 2)
		END AS PriceAmount,
		'EA' AS unitCode,
	...
	if($validaciones) {
		...
		$dolibarr->grabardatos($cadenaf);
		echo "<br />===> Error en en docuemnto: ".$prefijo.$numero."\n";
		echo var_dump($errores);
	}
}
if($nohayfacturas) { echo "<br /><font color='red'>**** NO SE ENVIO NINGUN DOCUMENTO **** </font> "; } else {
	echo "<br /><font color='blue'><h3>Los resultados se reflejaran en unos segundos...actualice este listado.</h3></font>";
}

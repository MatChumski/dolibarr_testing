...
INNER JOIN ".PREFIJO."c_departements AS depa
ON soc.fk_departement = depa.rowid
INNER JOIN ".PREFIJO."facture_extrafields AS extra
ON fac.rowid = extra.fk_object
INNER JOIN ".PREFIJO."c_paiement AS paiement
ON fac.fk_mode_reglement = paiement.id
INNER JOIN ".PREFIJO."c_payment_term AS payment
ON fac.fk_cond_reglement = payment.rowid
WHERE (NOT extra.envio OR extra.envio is NULL) AND fac.type = '3' AND LEFT(fac.ref,1) <> '(' AND (extra.isvalid <> 'true' OR extra.isvalid is NULL) AND fac.ref LIKE '".DOCUMENT."%' ORDER BY fac.ref
LIMIT ".LIMITE;
...
while($row = $result->fetch_array(MYSQLI_ASSOC)) {
	...
	if($validaciones) {
		...
		echo "<br />===> Documento ND: (".$prefijo.$numero.") enviando a la Dian,  ";
		...
		$dolibarr->grabardatos($cadenaf);
		echo "<br />===> Error en la NOTA DEBITO: ".$prefijo.$numero."\n";

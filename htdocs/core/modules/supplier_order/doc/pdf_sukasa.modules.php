...
/**
 *	\file       htdocs/core/modules/supplier_order/doc/pdf_sukasa.modules.php
 *	\ingroup    fournisseur
 *	\brief      File of class to generate suppliers orders from sukasa model
 */
...
/**
 *	Class to generate the supplier orders with the sukasa model
 */
class pdf_sukasa extends ModelePDFSuppliersOrders
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "sukasa";
		$this->description = "Plantilla para órdenes a proveedores Sukasa";
		...
		if ($conf->fournisseur->commande->dir_output) {
			...
			if (file_exists($dir)) {
				...
				if (empty($this->atleastonediscount)) {
					$delta = ($this->postotalht - $this->posxreten);
					$this->posxqty += $delta;
					$this->posxtva += $delta;
					$this->posxunit += $delta;
					$this->posxtvaVal += $delta;
					$this->posxreten += $delta;
					$this->posxtotalnet += $delta;
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					$value = round($object->lines[$i]->pu_ht, 2);

					$retencion = $object->lines[$i]->array_options['options_retefuente'];
					$retencionPercent;
					switch ($retencion){
						case 1:
							$retencionPercent = 2;
	...
	// phpcs:disable PEAR.NamingConventions.ValidFunctionName.PublicUnderscore
	/**
	 *   Show table for lines | Nombres de las columnas
	 *		
	 * 
	 *   @param		TCPDF		$pdf     		Object PDF

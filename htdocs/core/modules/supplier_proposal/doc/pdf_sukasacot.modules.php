 ...
 *	\ingroup    propale
 *	\brief      File of the class allowing to generate the supplier propals to the sukasacot model
 */
...
/**
 *	Class to generate PDF supplier proposal sukasacot
 */
class pdf_sukasacot extends ModelePDFSupplierProposal
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "sukasacot";
		$this->description = $langs->trans('DocModelsukasacotDescription');
		...
		*/
		...
		$this->posxtvaVal = 125 - 2 - 2;
		$this->widthtvaVal = 25 - 0.4;

		$this->posxreten = 150 - 2 - 2;
		$this->widthreten = 10 - 0.4;

		$this->posxretenVal = 160 - 2 - 2;
		$this->widthretenVal = 20;

		$this->posxtotalnet = 180 - 2 - 2; 
		$this->widthtotalnet = 25 - 0.4;
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
			...
			for ($i = 0; $i < $nblines; $i++) {
				...
				if (!empty($conf->global->PRODUCT_USE_OLD_PATH_FOR_PHOTO)) {
					$pdir = get_exdir(
						...
						$objphoto, 
						'product'
						).$object->lines[$i]->fk_product."/photos/";
		...
		if ($conf->supplier_proposal->dir_output) {
			...
			if (file_exists($dir)) {
				...
				" ".
				$outputlangs->transnoentities("CommercialAsk").
				" ".
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if (!empty($imglinesize['width']) && !empty($imglinesize['height'])) {
						...
						$pdf->Image(
							$realpatharray[$i], 
							$curX + (($this->posxtva - $this->posxpicture - $imglinesize['width']) / 2), 
							$curY, 
							$imglinesize['width'], 
							$imglinesize['height'], 
							'', 
					...
					if ($pageposafter > $pageposbefore) {	
						...
						$pageposafter = $pdf->getPage();
						$posyafter = $pdf->GetY();
						//var_dump($posyafter); var_dump(($this->page_hauteur - ($heightforfooter+$heightforfreetext+$heightforinfotot))); 
						//exit;
					...
					$value = round($object->lines[$i]->subprice, 2);

					$sql = "SELECT retefuente FROM llx_supplier_proposaldet_extrafields";
					$sql .= " WHERE fk_object = ". $object->lines[$i]->id;

					$result = $this->db->getRow($sql);
				
					$retencion = $result->retefuente;

					$retencionPercent = $retencion;
					switch ($retencion){
						case "1":
							$retencionPercent = 2;
							break;
						case "2":
							$retencionPercent = 2.5;
							break;
						case "3":
							$retencionPercent = 3.5;
							break;
						case "4":
							$retencionPercent = 4;
							break;
					...
					$retencionVal = $value * $retencionPercent/100;

					$ivapercent = $object->lines[$i]->tva_tx;
					$ivaval = $value * $ivapercent/100;

					$total = $value + $ivaval - $retencionVal;
					$this->total_neto += $total;
					...
					*/
					...
					$pdf->MultiCell($this->widthqty, 4, $qty, 0, 'R'); // Enough for 6 chars

					// IVA %

					$vat_rate = pdf_getlinevatrate($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxtva, $curY);
					$pdf->MultiCell($this->widthtva, 3, $vat_rate, 0, 'R');
					
					// PRECIO UNITARIO	

					$pdf->SetXY($this->posxunit, $curY);
					$pdf->MultiCell($this->widthunit, 4, '$' . price($value), 0, 'L');
					
					// IVA VALOR

					$vat_rate = pdf_getlinevatrate($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxtvaVal, $curY);
					$pdf->MultiCell($this->widthtvaVal, 3, '$' . price($ivaval), 0, 'R');

					// RETENCION %
					
					$pdf->SetXY($this->posxreten, $curY);
					$pdf->MultiCell($this->widthreten, 3, $retencionPercent . '%', 0, 'R');

					// RETENCION VALOR
					
					$pdf->SetXY($this->posxretenVal, $curY);
					$pdf->MultiCell($this->widthretenVal, 3, '$' . price($retencionVal), 0, 'R');

					// TOTAL NETO
					
					$pdf->SetXY($this->posxtotalnet, $curY);
					$pdf->MultiCell($this->widthtotalnet, 3, '$' . price($total), 0, 'R', 0);
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell($largcol2, $tab2_hl, '$' . price($this->total_neto), $useborder, 'R', 1);
		$pdf->SetFont('', '', $default_font_size - 1);
		$pdf->SetTextColor(0, 0, 0);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop)) {
			...
			$pdf->SetXY(
				$this->page_largeur - $this->marge_droite - ($pdf->GetStringWidth($titre) + 3), 
				$tab_top - 4
			);
		...
		*/
		...
		// CANTIDAD
		$pdf->line($this->posxqty - 1, $tab_top, $this->posxqty, $tab_top + $tab_height);
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxqty, $tab_top + 1);
			$pdf->MultiCell($this->widthqty, 2, 'Ct.', '', 'C');
		}

		// IVA %
		$pdf->line($this->posxtvaVal, $tab_top, $this->posxtvaVal, $tab_top + $tab_height);
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxtva, $tab_top + 1);
			$pdf->MultiCell($this->widthtva, 2, $outputlangs->transnoentities("VAT") . '%', 'C');
		}
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell($this->widthunit, 2, $outputlangs->transnoentities("PriceUHT"), '', 'C');
		}
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT)) {
			...
			if (empty($hidetop)) {
				$pdf->SetXY($this->posxtvaVal, $tab_top + 1);
				$pdf->MultiCell($this->widthtvaVal, 2, $outputlangs->transnoentities("VAT"), '', 'C');
			}
		}

		// RETENCION %
		$pdf->line($this->posxreten - 1, $tab_top, $this->posxreten - 1, $tab_top + $tab_height);
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxreten, $tab_top + 1);
			$pdf->MultiCell($this->widthreten, 2, "Rt.%", '', 'C');			
		}

		// RETENCION
		$pdf->line($this->posxretenVal - 1, $tab_top, $this->posxretenVal - 1, $tab_top + $tab_height);
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxretenVal, $tab_top + 1);
			$pdf->MultiCell($this->widthretenVal, 2, "Rt.", '', 'C');			
		}

		// TOTAL NETO
		$pdf->line($this->posxtotalnet, $tab_top, $this->posxtotalnet, $tab_top + $tab_height);
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxtotalnet, $tab_top + 1);
			$pdf->MultiCell($this->widthtotalnet, 2, "Total Neto", '', 'C');
		}
	}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($object->statut == 0 && (!empty($conf->global->SUPPLIER_PROPOSAL_DRAFT_WATERMARK))) {
			pdf_watermark(
				...
				$this->page_largeur, 
				'mm', 
				$conf->global->SUPPLIER_PROPOSAL_DRAFT_WATERMARK
			);
		}
		...
		if ($this->emetteur->logo) {
			if (is_readable($logo)) {
				...
				$pdf->MultiCell(
					100, 
					3, 
					$outputlangs->transnoentities("ErrorLogoFileNotFound", $logo), 
					0, 
					'L'
				);
				$pdf->MultiCell(
					100, 
					3, 
					$outputlangs->transnoentities("ErrorGoToGlobalSetup"), 
					0, 
					'L'
				);
			}
		...
		if ($object->ref_client) {
			...
			$pdf->MultiCell(
				100, 
				3, 
				$outputlangs->transnoentities("RefCustomer").
				" : ".
		...
		if ($object->thirdparty->code_fournisseur) {
			...
			$pdf->MultiCell(
				100, 
				3, 
				$outputlangs->transnoentities("SupplierCode").
				" : ".
		...
		if ($showaddress) {
			...
			if (count($arrayidcontact) > 0) {
				...
				$labelbeforecontactname = (
					$outputlangs->transnoentities("FromContactName") != 'FromContactName' ? 
			...
			if (!empty($usecontact)) {
				if ($usecontact && 
					($object->contact->socid != $object->thirdparty->id && 
					(!isset($conf->global->MAIN_USE_COMPANY_NAME_OF_CONTACT) || 
			...
			$carac_client = pdf_build_address(
				...
				$this->emetteur, 
				$object->thirdparty, 
				($usecontact ? $object->contact : ''), $usecontact, 'target', $object);

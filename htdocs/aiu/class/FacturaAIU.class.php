...
class FacturaAIU  
{
    ...
    function fetch($id)
    {
		...
		$sql.= " f.pedido_fk,";
		$sql.= " f.aiu_administracion_tipo,";
		$sql.= " f.aiu_administracion_valor,";
		$sql.= " f.aiu_imprevisto_tipo,";
		$sql.= " f.aiu_imprevisto_valor,";
		$sql.= " f.aiu_utilidad_tipo,";
		$sql.= " f.aiu_utilidad_valor";     
	...
	function update_products_aiu($linea_id_eliminar = NULL)
	{
		...
		{		
			...
			{
				$aiu_valor_administracion_aplicar = $importe_factura*$aiu_valor_administracion_aplicar/100;
			}		
			if($aiu_valor_administracion_aplicar > 0)
			{
				...
				if($producto_administracion->id > 0)
				{
					...
					$object->addline(
							$desc,$pu_ht,$qty,$tva_tx,0,0,$producto_administracion->id,0,'','',0,0,'',$price_base_type,$pu_ttc,
							$type,-1,0,'',0,0,null,0,$label,0
							);
				}
			}
			//**FIN ADMINISTRACION**
			...
			{
				$aiu_valor_imprevisto_aplicar = $importe_factura*$aiu_valor_imprevisto_aplicar/100;
			}
			...
			{					
				...
				if($producto_imprevisto->id > 0)
				{
					...
					$object->addline(
							$desc,$pu_ht,$qty,$tva_tx,0,0,$producto_imprevisto->id,0,'','',0,0,'',$price_base_type,$pu_ttc,
							$type,-1,0,'',0,0,null,0,$label,0
							);
				}
			}
			...
			{
				$aiu_valor_utilidad_aplicar = $importe_factura*$aiu_valor_utilidad_aplicar/100;
			}
			...
			{					
				...
				if($producto_utilidad->id > 0)
				{
					...
					$object->addline(
							$desc,$pu_ht,$qty,$tva_tx,0,0,$producto_utilidad->id,0,'','',0,0,'',$price_base_type,$pu_ttc,
							$type,-1,0,'',0,0,null,0,$label,0
							);
				}
			}
